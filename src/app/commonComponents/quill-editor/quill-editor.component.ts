...
import {
  ...
  export class QuillEditorComponent implements AfterViewInit, ControlValueAccessor, OnChanges, Validator {
    ...
    userList: any;
    constructor(private elementRef: ElementRef, @Inject(DOCUMENT) private doc: any, private renderer: Renderer2, private http: HttpClient, private routService: RouterService) {
    ...
    ngAfterViewInit() {
      ...
      var  toolbar = this.quillEditor.getModule('toolbar');
    ...
    saveToServer = (file: File)=> {
      ...
      fd.append('createUserId', this.userList.id);
      fd.append('webId', this.userList.webId);
      
      const xhr = new XMLHttpRequest();
      xhr.open('POST', this.routService.upLoadFile, true);
      xhr.onload = () => {
        if (xhr.status === 200) {
    ...
    insertToEditor(url: string) {
      ...
      this.quillEditor.insertEmbed(range.index, 'image', `http://localhost:9000${url}`);
    }
